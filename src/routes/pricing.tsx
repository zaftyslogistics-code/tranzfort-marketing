  ...
  return (
    <SiteLayout>
      ...
      <PageHero
        ...
        title={
          <>
            Free for the people who <span className="text-gradient-brand">move India</span>.
          </>
        }
          ...
          {plans.map((p) => (
            ...
            >
              ...
              <div className="mt-3 flex items-end gap-1.5">
                ...
                <div className={`text-xs pb-1.5 ${p.featured ? "text-white/70" : "text-ink-soft"}`}>
                  {p.period}
                </div>
              </div>
              <p className={`mt-3 text-sm ${p.featured ? "text-white/80" : "text-ink-soft"}`}>
                {p.desc}
              </p>
              <ul className="mt-5 space-y-2 flex-1">
                {p.features.map((f) => (
                  <li key={f} className="flex items-start gap-2 text-sm">
                    <Check
                      className={`h-4 w-4 mt-0.5 shrink-0 ${p.featured ? "text-orange-glow" : "text-teal"}`}
                    />
                    <span>{f}</span>
                  </li>
                ))}
              ...
              <Link
                ...
                className={`mt-7 inline-flex items-center justify-center gap-2 font-semibold px-4 py-2.5 rounded-full transition-transform hover:scale-[1.02] ${
                  p.featured ? "bg-white text-ink" : "bg-gradient-brand text-white shadow-brand"
                }`}
              >
      ...
      <section id="faq" className="py-20 bg-muted/40">
        <div className="max-w-4xl mx-auto px-5 lg:px-8">
          ...
          <div className="mt-10 space-y-3">
            {[
              {
                q: "Is the Driver plan really free forever?",
                a: "Yes. Our promise: TranZfort will always be free for individual drivers and suppliers using the core platform. We make our money on Fleet Pro and Enterprise plans.",
              },
              {
                q: "Do you take any commission on loads?",
                a: "No. Zero commissions, zero hidden charges. The price the supplier posts is the price the trucker gets.",
              },
              {
                q: "What's included in the 14-day Fleet Pro trial?",
                a: "Full access to the Pro dashboard for up to 5 trucks. No credit card required. After 14 days you choose whether to upgrade or stay on the free plan.",
              },
              {
                q: "Can I switch plans later?",
                a: "Anytime. Upgrade, downgrade or cancel from your account settings. Pro-rated refunds on annual plans.",
              },
              {
                q: "Is GST included in the prices?",
                a: "No, prices shown are exclusive of 18% GST. GST invoices are auto-generated for all paid plans.",
              },
              {
                q: "What's the Enterprise pricing?",
                a: "It depends on fleet size, integrations, and AI customisation. Talk to our sales team — most enterprise deployments start at ₹2-5 lakh per year.",
              },
            ].map((x, i) => (
              <details
                key={i}
      ...
      <section id="cta" className="py-20 bg-background">
        <div className="max-w-3xl mx-auto px-5 lg:px-8 text-center">
          ...
          <p className="mt-3 text-ink-soft">
            Our team can help you pick the right plan for your fleet.
          </p>
          <Link
            to="/contact"
            className="mt-7 inline-flex items-center gap-2 bg-gradient-brand text-white font-semibold px-6 py-3.5 rounded-full shadow-brand"
          >
