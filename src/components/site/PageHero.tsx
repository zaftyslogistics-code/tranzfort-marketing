...
export function PageHero({ eyebrow, title, desc, children }: {
  eyebrow: string; title: ReactNode; desc: string; children?: ReactNode;
}) {
  return (
    <section className="relative bg-gradient-dark text-white overflow-hidden">
      <div className="absolute inset-0 grid-pattern opacity-40" />
      <div className="absolute inset-0"
        style={{ background: "radial-gradient(ellipse 80% 50% at 50% 0%, oklch(0.55 0.11 195 / 0.4), transparent 60%)" }} />
      <div className="relative max-w-5xl mx-auto px-5 lg:px-8 py-24 lg:py-32 text-center">
        ...
        <h1 className="mt-5 text-5xl lg:text-6xl font-bold leading-tight tracking-tight">{title}</h1>
